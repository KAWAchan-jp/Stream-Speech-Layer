...
現在の実装では、音声認識結果を `chrome.storage.local` に `transcriptLog` として保存する。

保存件数は直近 50 件まで。新しい認識結果が追加されるたびに、古いものから削除して最新 50 件だけを残す。
...
popup の「最近の文字起こし」には、この保存ログを新しい順に表示する。あわせて `lastTranscript` と `lastTranslation` に最新の認識結果・翻訳結果を保存し、ページ上オーバーレイの復元に使う。
